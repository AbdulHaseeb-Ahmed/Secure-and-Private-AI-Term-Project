...
def getaccuracy_forone_extraction(x_test, y_test):
    ...
    return accuracies, x_test_adv_pre, x_test_adv_eps_5, x_test_adv_eps_10, x_test_adv_eps_50, x_test_adv_eps_95, y_test_adv
...
accu = []
for i in range(0, 5):
    result = getaccuracy_forone_extraction(x_test, y_test)
    accu.append(result[0])

final_accuracies = []
for i in range(0, 4):
    x = (accu[0][i] + accu[1][i] + accu[2][i] + accu[3][i]) / 4
